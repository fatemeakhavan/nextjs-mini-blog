...
export default function PostCard({ id, title, body, imageUrl }: PostCardProps) {
  const imageCard = imageUrl || `${process.env.NEXT_PUBLIC_IMAGE_URL}/400/200?random=${id}`
    ...
    <article className="w-full p-2">
      <Card className="hover:shadow-xl transition-shadow duration-300 flex flex-col h-full !py-0">
        ...
        <CardContent className="flex flex-col p-4 h-48">
          <h2 className="text-gray-900  line-clamp-1 text-lg font-semibold mb-2">{title}</h2>
          <p className="text-gray-700 h-12 line-clamp-3 md:line-clamp-4 lg:line-clamp-5 mb-2">{body}</p>
          <Link
            href={`/blog/${id}`}
